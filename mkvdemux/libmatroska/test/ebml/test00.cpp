...
/*!
    \file
    \version \$Id: test00.cpp,v 1.2 2003/04/28 10:06:10 robux4 Exp $
...
int main(void)
{
	...
	*(static_cast<EbmlUInteger *>(&MyDocTypeReadVer)) = 1;
	...
	KaxAttachements * pAllAttachements = static_cast<KaxAttachements *>(FirstSegment.FindFirstElt(KaxAttachements::ClassInfos, true));
	if (pAllAttachements == NULL)
		return -1;
	pAllAttachements->SetSizeInfinite();
	// size is unknown and will always be, we can render it right away
	pAllAttachements->Render(Ebml_file);
	
	KaxAttached * pAttachement1 = static_cast<KaxAttached *>(pAllAttachements->FindFirstElt(KaxAttached::ClassInfos, true));
	if (pAttachement1 == NULL)
		return -1;
	KaxFileName * pFileName1 = static_cast<KaxFileName *>(pAttachement1->FindFirstElt(KaxFileName::ClassInfos, true));
	if (pFileName1 == NULL)
		return -1;
	*static_cast<EbmlUnicodeString *>(pFileName1) ="file1.txt";
	KaxFileData * pFileData1 = static_cast<KaxFileData *>(pAttachement1->FindFirstElt(KaxFileData::ClassInfos, true));
	if (pFileData1 == NULL)
		return -1;
	...
	pAttachement1->Render(Ebml_file);
	
	KaxAttached * pAttachement2 = static_cast<KaxAttached *>(pAllAttachements->AddNewElt(KaxAttached::ClassInfos));
	if (pAttachement2 == NULL)
		return -1;
	KaxFileName * pFileName2 = static_cast<KaxFileName *>(pAttachement2->FindFirstElt(KaxFileName::ClassInfos, true));
	if (pFileName2 == NULL)
		return -1;
	*static_cast<EbmlUnicodeString *>(pFileName2) = "file2.txt";
	// Add a void element (data is discarded)
	EbmlVoid * pVoid = static_cast<EbmlVoid *>(pAttachement2->FindFirstElt(EbmlVoid::ClassInfos, true));
	if (pVoid == NULL)
		return -1;
	static_cast<EbmlBinary *>(pVoid)->SetBuffer((const binary*) Buffer1, countof(Buffer1));
	pAttachement2->Render(Ebml_file);
	...
	while (ElementLevel0 != NULL)
	{
		...
		while (ElementLevel1 != NULL) {
			...
			if (EbmlId(*ElementLevel1) == KaxAttachements::ClassInfos.GlobalId) {
				printf("Attachements detected\n");

				ElementLevel2 = aStream.FindNextID(KaxAttachements_Context, bUpperElement, 0xFFFFFFFFL, true);
			...
			ElementLevel1->SkipData(aStream, KaxAttachements_Context);
			delete ElementLevel1;
