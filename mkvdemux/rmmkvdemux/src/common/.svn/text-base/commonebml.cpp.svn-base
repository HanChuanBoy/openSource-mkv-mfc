/*
   ...
   $Id: commonebml.cpp 3415 2007-01-03 14:15:50Z mosu $
...
__debug_dump_elements(EbmlElement *e,
  ...
  if ((m = static_cast<EbmlMaster *>(e)) != NULL) {
    mxprint(stdout, " (size: %u)\n", (unsigned int)m->ListSize());
    for (i = 0; i < m->ListSize(); i++)
      debug_dump_elements((*m)[i], level + 1);
  } else if ((s = static_cast<EbmlString *>(e)) != NULL)
    mxprint(stdout, " (%s)\n", string(*s).c_str());
  else if ((us = static_cast<EbmlUnicodeString *>(e)) != NULL)
    mxprint(stdout, " (%s)\n", UTFstring_to_cstrutf8(UTFstring(*us)).c_str());
  else if ((si = static_cast<EbmlSInteger *>(e)) != NULL)
    mxprint(stdout, " (" LLD ")\n", int64(*si));
  else if ((ui = static_cast<EbmlUInteger *>(e)) != NULL)
    mxprint(stdout, " (" LLU ")\n", uint64(*ui));
  else if ((f = static_cast<EbmlFloat *>(e)) != NULL)
    mxprint(stdout, " (%f)\n", double(*f));
  else
    mxprint(stdout, "\n");
}
...
sort_ebml_master(EbmlMaster *m) {
  ...
  first_master = -1;
  for (i = 0; i < m->ListSize(); i++) {
    //if ((static_cast<EbmlMaster *>((*m)[i]) != NULL) &&
    if ((static_cast<EbmlMaster *>((*m)[i]) != NULL) &&
        (first_master == -1))
      first_master = i;
    //else if ((static_cast<EbmlMaster *>((*m)[i]) == NULL) &&
    else if ((static_cast<EbmlMaster *>((*m)[i]) == NULL) &&
  ...
  while (first_element != -1) {
    ...
    for (first_element++; first_element < m->ListSize(); first_element++)
      if (static_cast<EbmlMaster *>((*m)[first_element]) == NULL)
        break;
  ...
  for (i = 0; i < m->ListSize(); i++)
    if (static_cast<EbmlMaster *>((*m)[i]) != NULL)
      sort_ebml_master(static_cast<EbmlMaster *>((*m)[i]));
